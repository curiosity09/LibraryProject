...
public class BookDaoImplImpl extends GenericDaoImpl<Book> implements BookDao {

    private static BookDaoImplImpl instance;
    ...
    private BookDaoImplImpl() {
    }

    public static BookDaoImplImpl getInstance() {
        if (instance == null) {
            instance = new BookDaoImplImpl();
        }
        return instance;
    }
    ...
    public Book findBookById(int bookId) {
        ...
        return optionalBook.orElse(Book.builder().build());
    }

    @Override
    public List<Book> findBookByName(String bookName, int limit, int offset) {
        return findAllByParameter(bookName, limit, offset, ConnectionPoolImpl.getInstance().getConnection(), SQL_SELECT_BOOK_BY_NAME);
    }

    @Override
    public List<Book> findAllBook(int limit, int offset) {
        return findAll(ConnectionPoolImpl.getInstance().getConnection(), SQL_SELECT_ALL_BOOK_WITH_LIMIT_OFFSET,limit, offset);
    }

    @Override
    public List<Book> findBookByAuthorFullName(String authorFullName, int limit, int offset) {
        return findAllByParameter(authorFullName, limit, offset, ConnectionPoolImpl.getInstance().getConnection(), SQL_SELECT_BOOK_BY_AUTHOR_FULL_NAME);
    }

    @Override
    public List<Book> findBookByGenre(String genreName, int limit, int offset) {
        return findAllByParameter(genreName, limit, offset, ConnectionPoolImpl.getInstance().getConnection(), SQL_SELECT_BOOK_BY_GENRE);
    }

    @Override
    public List<Book> findBookBySection(String sectionName, int limit, int offset) {
        return findAllByParameter(sectionName, limit, offset, ConnectionPoolImpl.getInstance().getConnection(), SQL_SELECT_BOOK_BY_SECTION);
    }
    ...
    public List<Book> getByOrderId(int id, int limit, int offset) {
        return findAllByParameter(id, limit, offset, ConnectionPoolImpl.getInstance().getConnection(), SQL_SELECT_BOOK_BY_ORDER);
    }
